...
void LoRaMacSetSessionKeys(const uint8_t* nwkSKey, const uint8_t* appSKey) {
    printf("Em LoRaMacSetSessionKeys - vou armazenar as chaves\n");

    printf("Restoring NwkSKey: ");
    for (int i = 0; i < 8; i++) printf("%02X ", nwkSKey[i]);
    printf("\n");
...
void LoRaMacSetDevAddr(uint32_t addr) {
    printf("Em LoRaMacSetSessionKeys - vou armazenar addr\n");
    LoRaMacDevAddr = addr;
}

void LoRaMacSetFCntUp(uint32_t value) {
    printf("Em LoRaMacSetSessionKeys - vou armazenar as uplink counter\n");
    UpLinkCounter = value;
}

void LoRaMacSetFCntDown(uint32_t value) {
    printf("Em LoRaMacSetSessionKeys - vou armazenar downlink counter\n");
    DownLinkCounter = value;
}

uint8_t* GetLoRaMacNwkSKey() {
    return LoRaMacNwkSKey;
}

uint8_t* GetLoRaMacAppSKey() {
    return LoRaMacAppSKey;
}

uint32_t* GetLoRaMacDevAddr() {
